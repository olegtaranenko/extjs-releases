...
Ext.define('Ext.grid.feature.Grouping', {
        ...
        '<tpl if="needsWrap">',
                    ...
                    '<tpl if="isFirstRow">',
                        ...
                        '<div id="{groupId}" class="' + Ext.baseCSSPrefix + 'grid-group-hd {collapsibleCls}" tabIndex="0">',
                            '<div class="' + Ext.baseCSSPrefix + 'grid-group-title" style="{[groupTitleStyle]}">',
                                '{%values.groupHeaderTpl.applyOut(values.groupInfo, out, parent);%}',
                            '</div>',
                        '</div>',
                    '</tpl>',
            ...
            '{%',
                'if (values.isCollapsedGroup) {',
                    'me.skippedRows += me.getRecordGroup(values.record).children.length - 1;',
                '}',
            '%}',
        '<tpl else>',
            '{%this.nextTpl.applyOut(values, out, parent);%}',
    ...
    init: function(grid) {
        ...
        // If it's a local store we can build a grouped store for use as the view's dataSource
            ...
            // Group store collapses when this flag is set
